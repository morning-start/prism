...
基于 Unix Domain Socket / Windows 命名管道实现本机进程间通信，不走网卡、内核直传，延迟远低于 localhost TCP。由 Go 等主进程托管子进程，自动拉起、重启、资源清理。

**适用场景：** 单机私有化部署、低延迟需求、内网高性能调用、进程隔离部署

**优势：** 复用全部 HTTP 路由与协议逻辑，零业务代码改动，仅替换传输层。
...
| 语言 | 推荐方式 | 备注 |
| --- | --- | --- |
| Go | 专属封装 SDK | 一键切换 HTTP / IPC / WASM 三模式 |
...
- 完成主流大模型全量适配器适配（OpenAI、Claude、文心、讯飞、通义、Ollama、vLLM）
- 落地三运行模式，完成 Go 统一 SDK 封装
- 完善协议版本治理、自动化测试、行为一致性校验
